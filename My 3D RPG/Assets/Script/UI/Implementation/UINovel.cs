...
namespace ProjectChan.UI
{
    ...
    public class UINovel : UIWindow
    {
        ...
        /// <summary>
        /// 노벨 기획 데이터를 이용해서 UI를 세팅하는 메서드
        /// </summary>
        /// <param name="sdNovel"></param>
        public void SetNovel(SDNovel sdNovel)
        {
            // 이름, 대화 세팅
            speakerName.text = sdNovel.name;
            dialogue.text = sdNovel.kr;
            ...
            switch (sdNovel.charType)
            {
                ...
                case CharType.NPC:
                    ...
                    SetImageSize(sdNovel.charType);

                    // 초상화가 존재한다면
                    if (sdNovel.portraitPath.Length > 1)
                    {
                        ...
                        portrait.sprite = SpriteLoader.GetSprite
                            (Define.Resource.AtlasType.Portrait, sdNovel.portraitPath);
                    }
                    break;
